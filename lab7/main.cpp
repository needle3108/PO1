/*
Tematem zadanie jest napisanie listy cyklicznej. Lista ta opiera się na hierarchii klas, z abstrakcyjna klasa bazowa Node, po której dziedziczą klasy przechowujące parametry: int, float i std::string.

Kluczem do napisania takiej listy, jest trzymanie dwóch wskaźników:
na pierwszy i ostatni element listy. Nowe elementy dołączamy na koniec po ostatnim elemencie.

Klasa CyclicList przejmuje na siebie zarządzanie pamięcią.
Kopiowanie musi być głęboka kopią - tworzymy nowe obiekty na podstawie starych.
...
* Przeładowanie operatora << musi wystąpić dwa razy. Jeden raz powinien być zaimplementowany jako funkcja składowa.
* Klasy NodeInt, NodeFloat i NodeString moga byc w jednym pliku "NodeSpec.h". 
* Nie korzystamy z gotowych konetenerów (std::list itp)
*/
